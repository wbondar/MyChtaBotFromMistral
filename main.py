...
# Настройка веб-драйвера (для Railway важно указывать правильные пути)
...
# Проверяем существование ChromeDriver перед инициализацией
...
def schedule_messages(chat_id: int, context: ContextTypes.DEFAULT_TYPE) -> None:
    for day in ["monday", "tuesday", "wednesday", "thursday", "friday"]:
        schedule.every().__getattribute__(day).at("08:00").do(send_scheduled_message, context, chat_id, "Вставайте, Засранцы и давайте работайте!")
        schedule.every().__getattribute__(day).at("22:00").do(send_scheduled_message, context, chat_id, "Пора спать, Засранцы! Завтра на работу!")

    for day in ["saturday", "sunday"]:
        schedule.every().__getattribute__(day).at("09:00").do(send_scheduled_message, context, chat_id, "Спите еще? Вставайте завтракать!")
        schedule.every().__getattribute__(day).at("22:00").do(send_scheduled_message, context, chat_id, "Хватит маяться! Спать пора!")
